...
DEFAULT_DATASET_NAMES = ["COCO", "CelebA-HQ", "DIV2K", "Paris_StreetView"]


def _default_placeholder_roots() -> List[str]:
    candidates = [
        Path("/home/wangjingyu/newGMFlow/DiS-main/data/placeholder"),
        Path(__file__).resolve().parents[1] / "data" / "placeholder",
    ]
    for base in candidates:
        roots = [str(base / name) for name in DEFAULT_DATASET_NAMES if (base / name).exists()]
        if roots:
            return roots
    return [f"./data/placeholder/{name}" for name in DEFAULT_DATASET_NAMES]
...
def get_cover_and_secret_paths_from_config(config: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    ...
    if not cover_roots:
        cover_roots = _default_placeholder_roots()
    if not secret_roots:
        secret_roots = _default_placeholder_roots()
    all_cover = []
...
def get_train_dataloader(config: Dict[str, Any], rank: int = 0, world_size: int = 1, shuffle: bool = True) -> DataLoader:
    ...
    if not train_cover or not train_secret:
        cover_roots = data_cfg.get("cover_roots", _default_placeholder_roots())
        secret_roots = data_cfg.get("secret_roots", _default_placeholder_roots())
        train_cover = []
        for r in cover_roots:
...
def get_secret_train_dataloader(config: Dict[str, Any], batch_size: Optional[int] = None) -> DataLoader:
    ...
    if not train_secret:
        secret_roots = config.get("data", {}).get("secret_roots", _default_placeholder_roots())
        train_secret = []
        for r in secret_roots:
            train_secret.extend(collect_image_paths(r))
    data_cfg = config.get("data", {})
